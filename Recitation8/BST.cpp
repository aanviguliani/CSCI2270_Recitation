...
bool BST::isValidBSTHelper(Node* root)
{
    if(root == NULL)
        return true;
    if(root->left!=NULL && (getMaxValueNode(root->left))->key > root->key)
        return false;
    if(root->right!=NULL && (getMinValueNode(root->right))->key < root->key)
        return false;
    if(!isValidBSTHelper(root->left) || !isValidBSTHelper(root->right))
        return false;
    return true;
}
